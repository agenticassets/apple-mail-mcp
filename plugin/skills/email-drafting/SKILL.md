...
**Reply body length:** the native path waits for Mail's compose editor to finish processing the typed keystrokes before it saves, so an accepted `reply_body` is saved whole; there is no silent tail truncation to design around. There is one cap, applied **before** any Mail mutation: `reply_to_email` projects the chunked typing plus the editor-drain poll and returns the structured `REPLY_BODY_TYPING_BUDGET_EXCEEDED` error when that projection exceeds the documented 480-second ceiling, which admits roughly 114,000 characters of `reply_body`. Nothing is created when it fires, and an explicit `timeout` cannot lift it (`timeout` can only raise the budget above the projection, never below). The remedies are to shorten `reply_body`, send the long text as an attachment, or split it across replies; `compose_email` / `create_rich_email_draft` carry no such cap but produce a standalone message, not a native in-thread reply.
